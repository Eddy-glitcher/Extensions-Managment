...
import ThemeToggle  from "./components/themes/theme.tsx";
import { extensionsData } from './data/extensions.data.ts';
import type { Extension } from './interfaces/etension.interface.ts';
...
const filterExtensionsBy = (extensions: Extension[], filterBy : string)=>{
  const filter = filterBy.toLowerCase().trim();

  if(filter == 'active') return extensions.filter((ext)=> ext.isActive);
  if(filter == 'inactive') return extensions.filter((ext)=> !ext.isActive);
...
function App() {
  ...
  // Menu items
  const [menu, setMenu] = useState<{name : string; isActive: boolean}[]>(menuItems);

  const selectMenuItemFilter = (index: number)=> {
  ...
  const [allExtensions, setVisibleExtensions] = useState<Extension[]>([]);

  useEffect(()=>{
    const activeFilter = menu.find((menuItem)=>{
      return menuItem.isActive;
    });

    const filteredExtensions = filterExtensionsBy(extensions, activeFilter?.name || menu[0].name.toLowerCase().trim());
    setVisibleExtensions(filteredExtensions!);
  }, [menu, extensions]);

  // To set the active or inactive extension 
  const toggleExtension = (id : string)=>{
    setExtensions((prev)=> prev.map((extension)=>extension.id === id ? {...extension, isActive : !extension.isActive} : extension));
  };

  // Delete extension
  const [selectedExtensionId, setSelectedExtension] = useState<string>('');
  const deleteExtension = ()=> {
    setExtensions((prev=> prev.filter((extension)=> extension.id !== selectedExtensionId ))
  )};

  // Modal open/close
  const [isPopupActive, activeModal]= useState<boolean>(false);
    ...
    <>
      <section className="extensions">
        <section className='extensions__container'>
          <header className="extensions__bar">
              <svg
                  role="img"
                  aria-label="Extensions Logo"
                  ...
                  ></path><defs
                    ><clipPath id="a"
              ><path fill="#fff" d="M0 0h40v41H0z"></path></clipPath></defs></svg>
              <nav>
                <ThemeToggle />
              </nav>
          </header>
          ...
          <main className="content">
            <nav className="content__menu-bar">
              ...
              <ul>
                {
                  menu.map((menuItem, index) => (
                    ...
                    >
                      <button
                        onClick={()=>{ selectMenuItemFilter(index) }}
                        className={`content__menu-item ${menuItem.isActive ? "content__menu-item--active" : ""}`}>
                      {menuItem.name}
                      </button>
                    </li>
                  ))
            ...
            <section className="content__cards">
              {
                allExtensions.map((extension, index)=> 
                  <article className="content__card" key={index}>
                    <div className="content__card-header">
                      <img src={extension.icon} alt={`logo de ${extension.name}`}/>
                      <article>
                    ...
                    <div className="content__card-toggle">
                      <button aria-label={`Eliminar ${extension.name}`} className="content__card-remove-btn" onClick={()=>{activeModal(true); openModal(extension.id)}}>Remove</button>
                      <button
                        aria-pressed={extension.isActive}
                        aria-label={`Activar/desactivar ${extension.name}`}
                        className={`content__card-toggle-btn ${extension.isActive ? 'content__card-toggle-btn--active' : ''}`}
        ...
        <section 
          role='dialog'
          aria-modal="true"
          className='extensions__popup'>
            <div className="extensions__popup-content">
                <h3>Seguro que quiere eliminar esta extension?</h3>
                <button aria-label='Cancelar' className='extensions__popup-btn-cancel' onClick={()=> {activeModal(false)}}>Cancelar</button>
                <button aria-label='Eliminar' className='extensions__popup-btn-delete' onClick={()=> {deleteExtension(); activeModal(false)}}>Eliminar</button>
            </div>
        </section>
        }
      </section>
